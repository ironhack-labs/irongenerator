...
class FileProcessor {
  constructor({ verbose = false, templateDirectory, destinationDirectory, templateData } = {}) {
    this.queue = [];
  ...
  _log(destination) {
    if (this.verbose) console.log(`   ${chalk.cyan('created')}: ${normalizePath(destination)}`);
  }
  ...
  async _render({ template, to, data = {} }) {
    ...
    const contents = await this.constructor._readFile(join(templateDirectory, `${template}.ejs`));
